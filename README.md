...
[短线侠](https://duanxianxia.cn) 非官方 Python 数据接口与 opencode Skill，覆盖涨停播报、竞价异动、板块强度、资金流向、龙虎榜、连板天梯、情绪指标、个股异动解析等 30+ 数据端点；内置**免 token 免费降级通道**（账号过期/未续费时可用）与**盘后情绪信号**（情绪顶点→降仓提醒，冰点→建仓提示，回暖→加仓提示）。
